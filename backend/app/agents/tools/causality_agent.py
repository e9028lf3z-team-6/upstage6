...
3. 모든 설명(reason, from_event, to_event)은 반드시 한국어로 작성하라.
4. 수정 제안 금지
5. 말투/표현/안전성 판단 금지
6. [전체 맥락 요약]을 참고하여, 앞부분의 설정이 뒷부분에서 어긋나는지(개연성) 확인하라.
...
{{
  ...
  "issues": [
    {{
      ...
      "char_end": 0,
      "quote": "문제 구간 원문 인용",
      "reason": "독자 기준에서 왜 인과가 끊기는지 한국어로 간단히 설명",
      "from_event": "사건 A(요약) - 한국어로 작성",
      "to_event": "사건 B(요약) - 한국어로 작성",
      "confidence": 0.0
    }}
  ]
}}
